    ...
    for parola in lista_parole:
        if vistamappa==0:
            if len(parola)>10:
                parola=parola[50:]
                stato=""
                if parola.find("range")<70 and parola.find("range")>0:
                    parola=parola[parola.find("</span>")-3:]
                if parola.find("ottimo stato")>0:
                    stato = "ottimo"
                if parola.find("ristrutturat")>0:
                    stato = "ristrutturato"
                if parola.find("nuov")>0:
                    stato = "nuovo"
                if parola.find("recent")>0:
                    stato = "recente"
                '''
                #### per la definizione dello stato dell'immobile ####
                stato=parola[:parola.find("ottimo stato")]
                stato=parola[:parola.find("ristrutturat")]
                stato=parola[:parola.find("nuovo")]
                stato=parola[:parola.find("nuova")]
                ...
                ival=parola.find("#planimetry")
                if ival>0:
                    parola=parola[ival:]
                    locali=parola[25:parola.find("</div>")]
                    locali=locali.replace("<span>","")
                    locali=locali.replace("</span>","")
                else:
                    locali=0
                ival=parola.find("size")
                parola=parola[ival:]
                ifine=parola.find("</span>")
                ival=parola.find("<span>")
                mq=parola[ival:ifine]
                #mq=mq.replace("</span","")
                mq=mq.replace("<span>","")
                r=mq.find(" m")
                if r>0:
                ...
                if ival>0:
                    parola=parola[ival:]
                    piano=parola[20:parola.find("</div>")]
                    piano=piano.replace("\"","")
                ...
                res.append((prezzo, mq, prmq, locali, piano, pianodesc, stato))
        else:
            l=len(parola)
            if l>75:
